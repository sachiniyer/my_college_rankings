...
def valid_relation(relation: str) -> tuple:
    ...
    if len(parts) == 2:
        ...
        raise argparse.ArgumentTypeError(
            "%s must have the form attribute:relation" % relation
        )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Parse the DBLP XML file and convert it to CSV"
    )
    parser.add_argument(
        "xml_filename",
        action="store",
        type=existing_file,
        help="The XML file that will be parsed",
    ...
    parser.add_argument(
        ...
        action="store",
        type=existing_file,
        help="The DTD file used to parse the XML file",
        metavar="dtd_filename",
    )
    parser.add_argument(
        "outputfile",
        action="store",
    ...
    if parsed_args.relations:
        attr_rel = {
            attribute: relation for (attribute, relation) in parsed_args.relations
        }
        ...
        if len(attributes) == len(set(attr_rel.values())) == len(parsed_args.relations):
            ...
            print(
                "Will create relations for attribute(s): %s"
                % (", ".join(sorted(attributes)))
            )
        else:
            print(
                "error: argument --relations: The element attributes and relation names must be unique."
            )
            exit(1)
    else:
...
def parse_xml(
    xml_file,
    elements: set,
    output_files: Dict[str, csv.DictWriter],
            ...
            if elem.tag == current_tag:
                ...
                if annotate:
                    set_type_information(
                        element_types, current_tag, elem.tag, elem.text
                    )
                for key, value in elem.attrib.items():
                    ...
                    if annotate:
                        set_type_information(
                            element_types, current_tag, column_name, value
                        )
            root.clear()
    if annotate:
...
def write_annotated_header(
    array_elements: dict,
    element_types: dict,
    output_filename: str,
    neo4j_style: bool = False,
):
    (path, ext) = os.path.splitext(output_filename)
...
def generate_neo4j_import_command(
    elements: set, relations: set, relation_alias: dict, output_filename: str
):
    (path, ext) = os.path.splitext(output_filename)
    command = (
        'neo4j-admin import --mode=csv --database=dblp.db --delimiter ";" --array-delimiter "|" '
        "--id-type INTEGER"
    )
    for element in elements:
        command += ' --nodes:%s "%s_%s_header%s,%s_%s%s"' % (
            element,
            path,
            element,
            ext,
    ...
    for relation in relations:
        ...
        command += ' --relationships:%s "%s_%s_%s%s"' % (
            predicate,
            path,
            relation,
            predicate,
...
def write_relation_files(
    ...
    for column_name, relation in relations.items():
        ...
        output_path_relation = "%s_%s_%s%s" % (
            path,
            column_name,
        ...
        with open(
            output_path_relation, mode="w", encoding="UTF-8"
        ) as output_file_relation:
...
def start(args):
    if (
        args.xml_filename is not None
        and args.dtd_filename is not None
        ...
        output_files = open_outputfiles(
            elements, element_attributes, args.outputfile, args.annotate
        )
        array_elements = None
        element_types = None
        with open(args.xml_filename, mode="rb") as xml_file:
            ...
            if args.annotate:
                (relations, unique_id, array_elements, element_types) = parse_xml(
                    xml_file, elements, output_files, relation_attributes, annotate=True
                )
            else:
                relations, unique_id = parse_xml(
                    xml_file, elements, output_files, relation_attributes
                )
        ...
        if args.annotate and array_elements and element_types:
            ...
            write_annotated_header(
                array_elements, element_types, args.outputfile, args.neo4j
            )
            if args.neo4j:
                ...
                with open(
                    "neo4j_import.sh", mode="w", encoding="UTF-8"
                ) as command_file:
...
def programatic_main(xml_filename, dtd_filename, outputfile):
    args = argparse.Namespace()
    args.xml_filename = xml_filename
    args.dtd_filename = dtd_filename
